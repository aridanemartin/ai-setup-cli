...
1. **Check staged changes** — run `git diff --cached`; if empty, run `git status` to see what's unstaged
2. **Stage files if needed** — `git add <specific files>` (never `git add -A` without review)
